...
function Folderdatasection(props) {
  ...
  const handleDeleteFunction = async (fileName) => {
    const token = Cookies.get("token");
    const response = await fetch(
      `http://localhost:4000/folder-struc/file-delete?fileName=${fileName}`,
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
    ...
    <>
      ...
      >
        <Table sx={{ minWidth: 650 }} aria-label="simple table">
          ...
          <TableBody>
            {props.sidebarfiledata && props.sidebarfiledata.length > 0 ? (
              props.sidebarfiledata.map((row, index) => (
                <TableRow
                  key={index}
...
export default Folderdatasection;
